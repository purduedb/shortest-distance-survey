...
def plot_data_distribution(data_train, data_test, data_name, query_name, dir_name, label=""):
    ...
    if 'get_ipython' in globals():
        ...
        file_path = os.path.join(dir_name, f"histogram_distances_{data_name}_{query_name}{temp_label}.png")
        print_green(f"Saving plot: {file_path}")
...
def plot_learning_curves(history, n_batches, model_name, data_name, query_name, dir_name):
    ...
    if 'get_ipython' in globals():
        ...
        # Save the plot
        file_path = os.path.join(dir_name, f"loss_history_{model_name}_{data_name}_{query_name}.png")
        print_green(f"Saving plot: {file_path}")
        plt.savefig(file_path)
...
def plot_targets_and_predictions(predictions, targets, model_name, data_name, query_name, dir_name, label=""):
    ...
    if 'get_ipython' in globals():
        ...
        # Save the plot
        temp_label = f"_{label}" if label else ""
        file_path = os.path.join(dir_name, f"predictions_targets_{model_name}_{data_name}_{query_name}{temp_label}.png")
        print_green(f"Saving plot: {file_path}")
        plt.savefig(file_path)
...
def plot_targets_and_mre_boxplots(predictions, targets, model_name, data_name, query_name, dir_name, label="", num_buckets=5):
    ...
    if 'get_ipython' in globals():
        ...
        os.makedirs(dir_name, exist_ok=True)
        temp_label = f"_{label}" if label else ""
        file_path = os.path.join(dir_name, f"mre_boxplot_{model_name}_{data_name}_{query_name}{temp_label}.png")
        print_green(f"Saving plot: {file_path}")
