...
class EmbeddingNN(BaseModel):
    def __init__(self, num_nodes, embed_size=64, n_hidden_1=500, n_output=1, init_embeddings=None, aggregation_method="mean", normalize=True, max_distance=1.0):
        ...
        print(f"Initializing EmbeddingNN...")
        print(f"  - Number of nodes: {num_nodes}")
        print(f"  - Embedding size: {embed_size}")
        print(f"  - Hidden size: {n_hidden_1}")
        ...
        if init_embeddings is None:
            ...
            print(f"  - Node embeddings: randomly initialized, shape {init_embeddings.shape}")
        else:
            print(f"  - Node embeddings: from provided attributes, shape {init_embeddings.shape}")
        if normalize:
