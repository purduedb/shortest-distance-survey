...
class Path2vec(BaseModel):
    def __init__(self, G, embed_size=64, max_distance=1.0,
                 regularize=False, l1factor=1e-10,
                 use_neighbors=True, neighbor_count=1, alpha=0.01, sparse=False):
        """
        ...
        num_nodes = G.vcount()
        self.neighbor_map = {i: G.neighbors(i) for i in range(num_nodes)}
        print(f"Initializing Path2vec...")
        print(f"  - Number of nodes: {num_nodes}")
        print(f"  - Embedding size: {embed_size}")
        print(f"  - Max distance: {max_distance}")
        print(f"  - Use neighbors: {use_neighbors} (count={neighbor_count}, alpha={alpha})")
        print(f"  - Regularize: {regularize} (l1factor={l1factor})")
