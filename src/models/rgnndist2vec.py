...
class RGNNdist2vec(BaseModel):
    def __init__(self, n_input, n_hidden_1, n_hidden_2, layer_type, node_attributes, edge_attributes, max_distance=1.0, disable_edge_weight=True):
        ...
        print(f"Initializing RGNNDist2Vec...")
        print(f"  - Number of nodes: {node_attributes.shape[0]}")
        print(f"  - Node feature dim (GNN input): {n_input}")
        print(f"  - Hidden sizes: {n_hidden_1}, {n_hidden_2}")
        print(f"  - Max distance: {max_distance}")
        print(f"  - Layer type: {layer_type}")
        print(f"  - Disable edge weight: {disable_edge_weight}")
        ...
        if layer_type.lower() == "gcn":
            ...
            # edge_dim=1 wires GATConv to accept scalar edge weights as edge_attr (shape [E,1])
            edge_dim = None if disable_edge_weight else 1
            self.layer1 = GATConv(n_input, n_hidden_1, add_self_loops=True, fill_value='mean', edge_dim=edge_dim)
            self.layer2 = GATConv(n_hidden_1, n_hidden_2, add_self_loops=True, fill_value='mean', edge_dim=edge_dim)
        ...
        # persistent=False: tracks device (moved by .to()), excluded from `.pt` but included in `.jit.pt`
        self.register_buffer('cached_embeddings', torch.zeros(0, n_hidden_2), persistent=False)
    ...
    def build_geometric_data(self, node_attributes, edge_attributes, layer_type, disable_edge_weight):
        ...
        if disable_edge_weight or layer_type.lower() == 'sage':  # SAGE does not support edge weights
            print(f"Disabling edge weights...")
            edge_weight = None
        elif layer_type.lower() == 'gat':
            # GATConv receives edge weights via edge_attr (shape [E, 1]), not a scalar weight
    ...
    def encode(self, node_features, edge_index, edge_weight=None):
        """Encode node features into embeddings using GNN layers."""
        if self.layer_type.lower() == 'sage':
            # SAGEConv doesn't accept edge_weight
    ...
    def precompute_embeddings(self, batch_size=1024):
        """Encode all nodes and cache embeddings for fast inference.

        batch_size=k:    batched subgraph passes — processes k center nodes at a time,
                         peak memory proportional to k-hop subgraph size, not |E|.
        batch_size=None: single full-graph pass — O(|E|·d) peak memory, use only for small graphs.
        """
        device = self.node_features.device
        N = self.geometric_data.num_nodes
        out_dim = self.cached_embeddings.shape[1]

        if batch_size is None:
            with torch.no_grad():
                self.cached_embeddings = self.encode(
                    self.node_features, self.edge_index, self.edge_weight
                )
            return

        # GCN needs an extra hop so boundary nodes have correct degree normalization.
        num_hops = self.num_layers + 1 if self.layer_type.lower() == 'gcn' else self.num_layers

        # Allocate output on device; fill center-node rows batch by batch.
        emb_all = torch.zeros(N, out_dim, device=device)
        with torch.no_grad():
            for start in range(0, N, batch_size):
                center_nodes = torch.arange(start, min(start + batch_size, N), device=device)
                subgraph_nodes, subgraph_edge_index, mapping, edge_mask = k_hop_subgraph(
                    node_idx=center_nodes, num_hops=num_hops,
                    edge_index=self.geometric_data.edge_index,
                    num_nodes=N, relabel_nodes=True,
                )
                subgraph_features = self.geometric_data.x[subgraph_nodes]
                subgraph_edge_weight = (
                    self.geometric_data.edge_weight[edge_mask]
                    if self.geometric_data.edge_weight is not None else None
                )
                sub_emb = self.encode(subgraph_features, subgraph_edge_index, subgraph_edge_weight)
                # mapping[i] is the position of center_nodes[i] in subgraph_nodes,
                # so sub_emb[mapping] gives embeddings in the same order as center_nodes.
                emb_all[center_nodes] = sub_emb[mapping]

        self.cached_embeddings = emb_all

    def forward(self, x1, x2):
        if self.cached_embeddings.shape[0] == 0:
            raise ValueError("cached_embeddings is empty — call precompute_embeddings() before forward()")
        # Extract embeddings for the given node indices
        emb1 = F.embedding(x1, self.cached_embeddings)
        emb2 = F.embedding(x2, self.cached_embeddings)

        # Compute L1 norm
        # Manhattan (L1) distance; abs().sum() is NVFuser-fuseable unlike torch.norm(p=1)
        distances = torch.abs(emb1 - emb2).sum(dim=1, keepdim=True)
    ...
    def forward_with_embeddings(self, x1, x2, embeddings):
        """Forward pass with explicitly provided embeddings (used during training with subgraph embeddings)."""
        # Extract embeddings for the given node indices
        emb1 = F.embedding(x1, embeddings)
        emb2 = F.embedding(x2, embeddings)
    ...
    def subgraph_extraction(self, x1, x2, geometric_data, subgraph_node_map, num_layers):
        """Extracts the k-hop subgraph around the nodes in batch (x1, x2) from the full graph."""
        # Get unique nodes in the batch
        batch_nodes = torch.cat([x1, x2]).unique()

        # Extract k-hop subgraph around the nodes in x1 and x2
        if self.layer_type.lower() == 'gcn':
            # GCN needs +1 hop so 2-hop boundary nodes have correct degree for normalization.
            num_hops = num_layers + 1
        else:
            # GAT/SAGE don't use degree normalization so exact hops suffice.
            num_hops = num_layers
        subgraph_nodes, subgraph_edge_index, mapping, edge_mask = k_hop_subgraph(
            node_idx=batch_nodes, num_hops=num_hops,
    ...
    def subgraph_forward(self, x1, x2, subgraph_node_map):
        """Run a forward pass restricted to the k-hop subgraph of (x1, x2). Returns distance predictions."""
        # Extract subgraph for the current batch
        subgraph, x1_sub, x2_sub = self.subgraph_extraction(x1, x2, self.geometric_data, subgraph_node_map, self.num_layers)
        ...
        return self.forward_with_embeddings(x1_sub, x2_sub, sub_embeddings)

    def _train_step(self, x1, x2, y, criterion, optimizer, subgraph_node_map):
        optimizer.zero_grad()           # Clear gradients

        # Compute predictions via localized subgraph forward pass
        y_pred = self.subgraph_forward(x1, x2, subgraph_node_map)
    ...
    def fit(self,
        ...
        for epoch in range(epochs):  # Loop over epochs
            ...
            for idx, batch in enumerate(dataloader):  # Loop over batches
                ...
                loss = self._train_step(i, j, d_ij, criterion, optimizer, subgraph_node_map)
            ...
            if val_dataloader is not None:
                ...
                val_predictions, val_targets, _ = self.evaluate(
                    val_dataloader, device=device, profile_time=False, eval_mode=eval_mode
                )
    ...
    def evaluate(self,
        ...
        with torch.no_grad():
            if eval_mode == 'full':
                # Compute and cache full-graph embeddings once; reused across all batches.
                self.precompute_embeddings()
                if device.startswith("cuda"):
                    torch.cuda.synchronize()
                def get_outputs(i, j):
                    return self.forward(i, j)
            elif eval_mode == 'subgraph':
                ...
                def get_outputs(i, j):
                    return self.subgraph_forward(i, j, subgraph_node_map)
            else:
                raise ValueError(f"Unknown eval_mode '{eval_mode}'. Use 'full' or 'subgraph'.")
            ...
            if profile_time:
                ...
                for idx, (i, j, d_ij) in enumerate(dataloader):
                    ...
                    outputs = get_outputs(i, j)             # Model inference
...
class RGNNdist2vecJIT(torch.nn.Module):
    """TorchScript-compatible inference wrapper for RGNNdist2vec.

    PyG's GATConv/GCNConv use Adj = Union[Tensor, SparseTensor] in their forward
    signatures, which produces a TorchScript type mismatch when the model is saved
    and reloaded (pyg-team/pytorch_geometric#8867, unfixed as of PyG 2.6.1). This
    class holds only the pre-computed embedding matrix and max_distance, keeping
    the GNN layers out of the serialized module tree entirely.

    Built by save_jit_model() after calling precompute_embeddings() on the trained
    RGNNdist2vec. The C++ benchmark skips precompute_embeddings() when absent and
    calls forward() directly.
    """
    def __init__(self, embeddings, max_distance):
        super().__init__()
        self.register_buffer('cached_embeddings', embeddings)
        self.max_distance = max_distance
