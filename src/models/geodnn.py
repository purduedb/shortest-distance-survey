...
class GeoDNN(BaseModel):
    def __init__(self, n_input, n_hidden_1, n_hidden_2, n_hidden_3, n_output, node_attributes, max_distance=1.0):
        ...
        print(f"Initializing GeoDNN...")
        print(f"  - Number of nodes: {node_attributes.shape[0]}")
        print(f"  - Node attribute dim: {node_attributes.shape[1]}")
        print(f"  - FC input size (concatenated src+dst attributes): {n_input}")
        print(f"  - Hidden sizes: {n_hidden_1}, {n_hidden_2}, {n_hidden_3}")
