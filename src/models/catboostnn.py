...
class CatBoostNN(BaseModel):
    def __init__(self, num_nodes, coordinate_embs, landmark_embs, max_distance=1.0):
        ...
        print(f"Initializing CatBoostNN...")
        print(f"  - Number of nodes: {num_nodes}")
        print(f"  - Number of landmarks: {self.num_landmarks}")
        print(f"  - Coordinate embedding dim: {coordinate_embs.shape[1]}")
    ...
    def encode(self, x1, x2):
        ...
        # Euclidean (L2) distance; square().sum().sqrt() is NVFuser-fuseable unlike torch.norm(p=2)
        euclidean_dist = (x1_coord_emb - x2_coord_emb).square().sum(dim=-1, keepdim=True).sqrt()
