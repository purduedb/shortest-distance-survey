...
class Vdist2vec(BaseModel):
    def __init__(self, n_input, n_hidden_1, n_hidden_2, n_hidden_3, n_output, max_distance=1.0, sparse=False):
        super().__init__()
        self.max_distance = max_distance
        print(f"Initializing Vdist2vec...")
        print(f"  - Number of nodes: {n_input}")
        print(f"  - Embedding size: {n_hidden_1}")
        print(f"  - Hidden sizes: {n_hidden_2}, {n_hidden_3}")
        print(f"  - Output size: {n_output}")
        print(f"  - Max distance: {max_distance}")
        print(f"  - Sparse: {sparse}")

        ## Define layers
        # Embedding layer
        self.embedding = nn.Embedding(n_input, n_hidden_1, sparse=sparse)
