...
for i, dataset in enumerate(FIGSHARE_DATASETS):
    print(f"[{i+1}/{len(FIGSHARE_DATASETS)}] Processing dataset: {dataset}")
...
for i, dataset in enumerate(DIMACS_DATASETS):
    print(f"[{i+1}/{len(DIMACS_DATASETS)}] Processing dataset: {dataset}")
    graph_file_name, coordinates_file_name = download_dimacs_data(DATA_DIR, dataset)
    fast_preprocess_dimacs(graph_file_name, coordinates_file_name, os.path.join(DATA_DIR, dataset))
