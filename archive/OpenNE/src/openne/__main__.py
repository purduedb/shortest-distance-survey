...
def main(args):
    ...
    print(f"Graph read.:")
    print(f"  - No. of nodes: {g.G.number_of_nodes()}")
    print(f"  - No. of edges: {g.G.number_of_edges()}")
    print(f"  - First 5 nodes: {list(g.G.nodes(data=True))[:5]}")
    print(f"  - First 5 edges: {list(g.G.edges(data=True))[:5]}")

    t1 = time.perf_counter()
    if args.method == 'node2vec':
        model = node2vec.Node2vec(graph=g,
                                  path_length=args.walk_length, num_paths=args.number_walks,
    ...
    precomputation_time = t2 - t1
    print(f"Total time: {precomputation_time / 60:.2f} minutes")
    if args.method != 'gcn':
        if args.output is None:
            file_name = f"{args.method}_{args.representation_size}.embeddings"
            args.output = os.path.join(os.path.dirname(args.input), file_name)
        print("Saving embeddings: ", args.output)
        model.save_embeddings(args.output, comment=f"# precomputation time={precomputation_time / 60:.2f} minutes")
