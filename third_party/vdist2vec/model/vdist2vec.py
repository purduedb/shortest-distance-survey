...
def get_node(index):
    node1_index = index // (n-1)  # integer division `/` -> `//` in python 3
...
def multilayer_perceptron(x1, x2, weights, biases):
    ...
    # print("weights shape: ", weights['h1'].shape)
    # embedding layer
    # x1 is a one-hot vector of node1
    # convert it to index of node1
    x1 = tf.argmax(x1, 1)
    x2 = tf.argmax(x2, 1)
...
# Training cycle
for epoch in range(training_epochs):  # xrange -> range in python 3
    print("Epoch: ", epoch)
    avg_cost = 0.
