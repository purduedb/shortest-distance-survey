This is a copy of original [OpenNE github repository](https://github.com/thunlp/OpenNE) with modifications to make it compatible with Python 3.10+ and tensorflow 2.x.
To test the code, you can run the following command in the OpenNE source directory:

```bash
# Change to the OpenNE source directory
cd third_party/OpenNE/src

# Run the node2vec method on sample data
python -m openne --method node2vec --input ../data/karate/karate.edgelist --representation-size 64 --workers 1 --epochs 1

# Run the LINE method on sample data
python -m openne --method line --input ../data/karate/karate.edgelist --representation-size 64 --order 2 --epochs 1

# Run the DeepWalk method on sample data
python -m openne --method deepWalk --input ../data/karate/karate.edgelist --representation-size 64 --workers 1 --epochs 1
```
