...
import tensorflow.compat.v1 as tf  # Changed import
tf.disable_v2_behavior()  # Added this line
...
class _LINE(object):
    ...
    def __init__(self, graph, rep_size=128, batch_size=1000, negative_ratio=5, order=3):
        ...
        initializer = tf.glorot_normal_initializer(seed=cur_seed)
...
class LINE(object):
    ...
    def save_embeddings(self, filename, delimiter=',', comment='#'):
        ...
        fout.write(f"{comment}, num_nodes={node_num}, vector_size={self.rep_size}\n")
        sorted_nodes = sorted(self.vectors.keys(), key=lambda x: int(x.strip()))
        for node in sorted_nodes:
            vec = self.vectors[node]
            fout.write(f"{node}{delimiter}{delimiter.join([str(x) for x in vec])}\n")
        fout.close()
