...
if seed is not None:
    print("Seed is set to be %d"%seed)
    np.random.seed(seed)
    random.seed(seed)
...
weights = {
    ...
    # Weight factors for combining outputs
...
def multilayer_perceptron(x1, x2, weights, biases):
    # embedding layer
    layer_11 = tf.add(tf.gather(weights['h1'], x1),biases['b1'])  ## WHY ADD BIAS HERE? (but gives slightly better results)
    # layer_11 = tf.gather(weights['h1'], x1)
    layer_12 = tf.add(tf.gather(weights['h1'], x2),biases['b1'])
    # layer_12 = tf.gather(weights['h1'], x2)

    # fusion of embeddings, concat operation
    ...
    # Combine outputs: lambda1*a1 + lambda2*a2 + lambda3*a3 + lambda4*a4
...
print("values of v1, v2, v3, v4:", sess.run([weights['v1'], weights['v2'], weights['v3'], weights['v4']]))
