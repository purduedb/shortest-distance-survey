#!/bin/bash
# Run the src/inference.py latency+throughput benchmark for every model in a
# given directory, for a single --backend/--device combo (one call = one
# combo, matching run_cpp_benchmark.sh's API). Minimal adaptation of
# catboost_inference_expt.sh: scans --model_dir for per-model checkpoints (.onnx
# for tensorrt, .jit.pt for libtorch) and runs src/inference.py against each.
# tensorrt + --device cpu uses ONNX Runtime's CPUExecutionProvider only.
#
# Usage:
#   bash inference_expt.sh --model_dir <dir>              # dry run
#   bash inference_expt.sh --model_dir <dir> -e           # run locally
#   bash inference_expt.sh --model_dir <dir> -s           # submit to SLURM (dry run)
#   bash inference_expt.sh --model_dir <dir> -s -e        # submit to SLURM and execute
...
MODEL_DIR_ARG=""                        # Dir containing saved_onnx_models/ and saved_jit_models/ (required)
...
BACKEND_ARG="tensorrt"                  # tensorrt | libtorch
DEVICE_ARG="cuda"                       # cuda | cpu
FILTER_DATASETS=""                      # Optional: comma-separated datasets to run, e.g. W_Jinan,W_Beijing
FILTER_MODELS=""                        # Optional: comma-separated models to run, e.g. GAT,GCN
OUTPUT_DIR_ARG=""                       # Optional: --output_dir passthrough (default: saved_inference_metrics/ sibling of model_dir)
...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        ...
        --slurm|-s)           SLURM=true ;;
        --model_dir)          MODEL_DIR_ARG="$2";      shift ;;
...
if [[ -z "$MODEL_DIR_ARG" ]]; then
    echo "Error: --model_dir is required (expects saved_onnx_models/ and saved_jit_models/ as siblings)"
    exit 1
fi
if [[ "$BACKEND_ARG" != "tensorrt" && "$BACKEND_ARG" != "libtorch" ]]; then
    echo "Error: --backend must be tensorrt or libtorch"
    exit 1
fi

MODEL_DIR_ARG="$(realpath "$MODEL_DIR_ARG")"
...
ONNX_DIR="$MODEL_DIR/saved_onnx_models"
JIT_DIR="$MODEL_DIR/saved_jit_models"
...
RUN_NAME="inference_jobs"
LOG_DIR="$MODEL_DIR/$RUN_NAME"
if [[ -n "$SLURM_JOBID" ]]; then
    MASTER_LOG="$LOG_DIR/inference-master-${SLURM_JOBID}.log"
else
    MASTER_LOG="$LOG_DIR/inference-master.log"
fi

if [[ -z "$OUTPUT_DIR_ARG" ]]; then
    OUTPUT_DIR_ARG="$MODEL_DIR/saved_inference_metrics"
fi

case "$BACKEND_ARG" in
    tensorrt) EXT="onnx";   MODEL_SUBDIR="$ONNX_DIR" ;;
    libtorch) EXT="jit.pt"; MODEL_SUBDIR="$JIT_DIR" ;;
esac
...
# Known query-dir strings in checkpoint filenames, <Model>_<Dataset>_<QueryDir>.{onnx,jit.pt},
...
# Canonical model names, matching saved_onnx_models/ + saved_jit_models/ basenames.
MODELS=(
    ## LpNorm
    'Manhattan'
    # 'Euclidean'
    ...
    ## Landmark
    'Landmark_random_subset'
    'Landmark_kmeans_subset'
    ...
    # CatBoost (CatBoost itself has no JIT/ONNX model -- use catboost_inference_expt.sh)
    'CatBoostNN'
)

if [[ -n "$FILTER_MODELS" ]]; then
    IFS=',' read -ra MODELS <<< "$FILTER_MODELS"
fi
...
if [ "$SLURM" = true ]; then
    MASTER_LOGFILE="$LOG_DIR/inference-master-%j.log"
    ...
    SBATCH_COMMAND="$SBATCH_COMMAND --model_dir $MODEL_DIR_ARG"
    ...
    SBATCH_COMMAND="$SBATCH_COMMAND --backend $BACKEND_ARG"
    SBATCH_COMMAND="$SBATCH_COMMAND --device $DEVICE_ARG"
...
# ---------------------------------------------------------------------------
# Run benchmark for every (dataset, model) pair, at the fixed backend/device
# ---------------------------------------------------------------------------
SECONDS=0
COUNTER=0
SKIPPED=0
...
echo "Running $TOTAL (dataset, model) combination(s) against MODEL_DIR=$MODEL_DIR"
echo "----------------------------------------"
...
for dataset in "${DATASETS[@]}"; do
    for model_name in "${MODELS[@]}"; do
        ...
        # Find the model file for this (model, dataset) pair
        model_path=""
        for qd in "${QUERY_DIRS[@]}"; do
            candidate="$MODEL_SUBDIR/${model_name}_${dataset}_${qd}.${EXT}"
            [ -f "$candidate" ] && { model_path="$candidate"; break; }
        done
        if [ -z "$model_path" ]; then
            echo "  SKIP: no $EXT file for model=$model_name dataset=$dataset"
            echo "----------------------------------------"
            SKIPPED=$((SKIPPED + 1))
            continue
        fi
