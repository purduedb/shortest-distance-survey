...
void preprocess_json(std::string filename, std::string output_name, unsigned int seed = 42){
    Graph g;
...
void preprocess_gilbreth(std::string graph_path,std::string output_name, unsigned int seed = 42){
    Graph g;
    ...
    cout << "threads: " << MULTI_THREAD_DISTANCES << endl;
    cout << "seed: " << seed << endl;
    ...
    double index_size_mb = (double)con_index.size() / MB;
    cout << fixed << setprecision(1) << "created index of size " << index_size_mb << " MB in " << result.index_time << "s" << endl;
    cout << fixed << setprecision(1) << "build time: " << result.index_time << " s" << endl;
    cout << fixed << setprecision(1) << "index size: " << index_size_mb << " MB" << endl;
    // write index
    if(std::filesystem::exists(output_name)){
        if(std::filesystem::remove(output_name)){
            std::cout<<"remove old index file"<<std::endl;
        }else{
            std::cerr<<"fail to remove the old file"<<std::endl;
        }
    }
    ofstream index_file;
    index_file.open(output_name, ios::out | ios::binary);
    con_index.write(index_file);
    index_file.close();
}

void compute_ground_truth(std::string graph_path, std::string query_path, std::string output_name, unsigned int seed = 42){
    Graph g;
    cout << endl << "reading graph from " << graph_path << endl;
    fstream fs(graph_path.c_str());
    read_dense_graph(g, fs,true);
...
int main(int argc, char** argv)
{
    ...
    if(argc<3){
        cerr << "Usage: " << argv[0] << " <graph_file> <output_file> [seed=42]" << endl;
        return 1;
    }
    unsigned int seed = (argc >= 4) ? (unsigned int)atoi(argv[3]) : 42;
    preprocess_gilbreth(argv[1], argv[2], seed);
